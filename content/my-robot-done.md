...
Потом я полумал что неплохо бы иметь защинту для камерв и ограничитель хода рамки,
...
Вторым шагом было проектирование черепа
Основной идеей было иметь голову, похожую на человеческую, но чтобы при этом 
в ней было достаточно места для устновки некоторого числа плат.

Прототип черепа был выпилен сначала из обчного экструдированного пенополистирола,
причем тут она продается оклееная бумагой с 2 сторон, и толщиной 6 мм - то что доктор
прописал.

То есть белый череп на видео как раз сделан из пеноплистирола, это просто модель.
...
Потом я повторил процедуру и выпилил уже каркас черепа из поликарбоната.
...
человекоподобным, то я постарался сохранять пропорции.
...
Внутрення часть напечатана из PLA, а внешняя из ABS но он мне не нравится, придется с ним
что-то делать, очень уж он хрупкий. Может быть заменю его в дальнейшем на PLA.
...
То есть телега была выбрана по нескольким причинам. 

Во-первых, она является само по себе опорой для всего робота и никакого подвеса не надо будет делать.

Во-вторых, пол в квартире ровный, без порогов, так что для перемещения на колесах
будет тратиться меньше энергии чем у бипеда (зависит от ситуации, сложный вопрос).

В-третих, в ящик можно будет засунуть батареи достаточно большой емкости, чтобы обеспечить
работу робота в течение например 10 часов без подзарядки.

Еще раз заострю внимание, что телега это вариант нижней части, которая может быть 
...
Передавать вращения я решил с помощью зубчатых ремней, с шагом 5мм, для этого напечатал
шкивы.

<img src="http://i.imgur.com/HOfAO00.jpg" alt="Шкив" width="600"/>

Шкив надевается на колесо и зажимается хомутом. То есть держится за счет трения
и никаких дополнительных изменений в колесах я не делал.

<img src="http://i.imgur.com/mqnAdpb.jpg" alt="Шкив на колесе" width="600"/>

Моторчик с редуктором выдает 150 rpm, закрепляется в таком вот кронштейне
...
У меня было несолько вариантов редукторов, но для старта я решил спроектировать 
...
Едиственное, что меня смущает, какой момент сил выдержат пластиковые шестерни.
